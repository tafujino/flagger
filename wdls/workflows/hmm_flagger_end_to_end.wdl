...
workflow HMMFlaggerEndToEnd{
    ...
    parameter_meta {
        ...
        augmentCoverageThreadCount : "Number of threads for augmentCoverageByLabels/augmentCoverageByLabelsConservative (Default : 8, matching the task's own upstream default). augment_coverage_by_labels's multithreaded chunk parser had two bugs that could corrupt memory or fail entirely at --threads>1 (see internal fork commits fixing cov_fast_reader.c/track_reader.c/chunk.c, and upstream https://github.com/mobinasri/flagger/issues/44 and related reports); both are fixed at the C level in this fork's docker image, so the full thread count is safe again. Lower this only if you suspect a regression."
