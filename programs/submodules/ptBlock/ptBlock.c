...
void ptBlock_save_in_bed(stHash *blocks_per_contig, char *bed_path, bool print_count_data) {
    ...
    for (int i = 0; i < stList_length(contigs); i++) {
        ...
        for (int j = 0; j < stList_length(blocks); j++) {
            ...
            if (print_count_data) {
                fprintf(fp, "%s\t%d\t%d\t%d\n", contig, block->rfs, block->rfe + 1,
                        *((int *) block->data)); // end should be 1-based in BED
            } else {
            ...
            while (sam_itr != NULL) {
                ...
                *total_read_count += 1;
                ...
                if (count_parsed_reads % 100000 == 0) {
                    fprintf(stderr, "[%s][%s:%d-%d] Parsed %d reads.\n", get_timestamp(), ctg_name, block->rfs,
                            block->rfe + 1, count_parsed_reads);
                }
...
stHash *ptBlock_multi_threaded_coverage_extraction(char *bam_path,
    ...
    for (int i = 0; i < stList_length(block_batches); i++) {
        ...
        fprintf(stderr, "[%s] Created thread for parsing batch %d (length=%ld)\n", get_timestamp(), i,
                ptBlock_get_total_length_by_rf(batch));
    }
    tpool_wait(tm);
    tpool_destroy(tm);
...
stHash *ptBlock_multi_threaded_coverage_extraction_with_zero_coverage_and_annotation(char *bam_path,
    ...
    if (MAX_NUMBER_OF_ANNOTATIONS < stList_length(annotation_block_table_list)) {
        fprintf(stderr,
                "[%s] Warning: %d annotation bed files are given, which is more than maximum number (%d). In the current implementation it may interfere with digits dedicated for coverage bias detection.\n",
...
void ptBlock_set_region_indices_by_mapping(stHash *blocks_per_contig, int *annotation_to_region_map,
